...
vector<UserStruct> SearchForUser(vector<UserStruct> arr, string str, int task) {
	...
	for (int i = 0; i < arr.size(); i++) {
		if (arr[i].login == str) {
			...
			case 1:
				ChangeColor(12);
				printf_s("Confirm deletion: press 1 to continue or any other key to cancel. ");
				ChangeColor(15);
				choice = _getch();
				if (choice != '1')
					break;
	...
	if (found == 0) {
		printf_s("No user with such login!\n");
	}
	return arr;
}
...
void DeleteUser() {
	...
	arr = ReadUsersFromFile();
	string str;
	printf_s("Enter login of a user to delete: ");
	cin >> str;
...
void ActivateUser() {
	...
	string str;
	printf_s("Enter login of an account to activate: ");
	cin >> str;
...
void BlockUnblockUser() {
	...
	arr = ReadUsersFromFile();
	string str;
	printf_s("Enter login of an account you would like to block/unblock: ");
	cin >> str;
	...
	while (1) {
		...
		case '1':
			...
			case '6':
				break;
			case '7':
				printf_s("Press 1 to exit or other key to stay: ");
				choice = _getch();
				if (choice == '1')
					return;
				break;
		...
		case '2':
			...
			printf_s("1 - Show data about calls\n 2 - Add new call data\n 3 - Edit call data\n 4 - Delete call\n 5 - Filter calls\n 6 - Sort calls\n 7 - Individual task\n 8 - Search calls\n 9 - Back\n 0 - Exit\n ");
			choice = _getch();
			switch (choice) {
			case '1':
				printf_s("\n");
